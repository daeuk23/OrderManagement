...
int main(int argc, char** argv)
{
	...
	std::vector<order::Workstation*> theStations;
	std::vector<order::CustomerOrder> theOrders;

	try {
		order::Utilities::setDelimiter(',');
		...
		::loadFromFile<order::CustomerOrder>(argv[3], theOrders);
		...
		for (auto& order : theOrders) {
			...
			order::g_pending.push_back(std::move(order));
		}
		std::cout << std::endl << std::endl;
		...
		std::cout << "========================================" << std::endl;
		order::LineManager lm(argv[4], theStations);
		lm.display(std::cout);
		std::cout << std::endl << std::endl;
	...
	std::cout << "========================================" << std::endl;
	for (const auto& o : order::g_completed)
		o.display(std::cout);
	std::cout << std::endl << std::endl;
	...
	std::cout << "========================================" << std::endl;
	for (const auto& o : order::g_incomplete)
		o.display(std::cout);
	std::cout << std::endl << std::endl;
	...
	std::cout << "========================================" << std::endl;
	for (const order::Station* theItem : theStations)
